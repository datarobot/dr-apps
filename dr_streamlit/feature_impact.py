...
def derived_features_chart(project_id: str, model_id: str, selected_class: Optional[str] = None, height: Any = None, width: Any = None):
    ...
    if project.target_type == TARGET_TYPE.MULTICLASS:
        ...
        c = alt.Chart(feature_impact, height=height if height else Undefined, width=width if width else Undefined).mark_bar().encode(y=alt.Y('featureName', sort='-x'), x=alt.X('impactNormalized'))
