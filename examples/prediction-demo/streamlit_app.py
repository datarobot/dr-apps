...
def intro():
    ...
    if not deployment_id:
        st.error(
            'The deploymentid environment variable must be set to use the predictor demo', icon='🚨'
        )
    if not project_id:
...
def predictor_app():
    ...
    pred = create_prediction_form(
        deployment, deployment.model['project_id'], use_batch_prediction_api=use_bp_api
    )
    if pred:
        ...
        if project.target_type == TARGET_TYPE.BINARY:
            ...
            prediction = next(
                ...
                if pred_value['label'] == project.positive_class
            )
            ...
            prediction = next(
                pv['value']
                for pv in pred['data'][0]['prediction_values']
                if pv['label'] == predicted_class
            )
        else:
...
def main():
    try:
        ...
        if not os.getenv('token'):
            st.error(
                "Please make sure the 'token' environment variable is set to be a valid token from your DataRobot account",
                icon='🔐',
            )
